...
const parseInput = (rawInput: string, isPart2: boolean) => {
	const [instructions, parts] = rawInput
		.split("\n\n")
		.map(info => info.split("\n"));

	const part1Workflows: Map<string, Part1Function> = new Map();
	const part2Workflows: Map<
		string,
		Array<{
			varName: string;
			operator: string;
	...
	instructions.forEach(line => {
		...
		if (!isPart2) {
			...
			part1Workflows.set(
				name,
				createPart1Function(part1Expressions.join("\r\n")),
			);
		} else {
			const part2Expressions = info[1]
				.substring(0, info[1].length - 1)
				.split(",");
...
const solve = (rawInput: string, isPart2: boolean) => {
	...
	if (!isPart2) {
		...
		return accepted.reduce(
			(acc, part) => acc + part.x + part.m + part.a + part.s,
			0,
		);
	} else {
		let part2Answer = 0;
		...
		while ((currentPart2 = part2Queue.shift()!) != undefined) {
			if (currentPart2.result == "A") {
				...
				currentPart2.part.isValid &&
				currentPart2.result != "R"
			) {
				const conditions = input.part2Workflows.get(
					currentPart2.result,
				)!;
				conditions.forEach(e => {
					...
					if (e.operator == "<") {
						(newPart as any)[e.varName + "Max"] = Math.min(
							(newPart as any)[e.varName + "Max"],
							e.value - 1,
						);
						(currentPart2.part as any)[e.varName + "Min"] =
							Math.max(
								(currentPart2.part as any)[e.varName + "Min"],
								e.value,
							);
					} else if (e.operator == ">") {
						(newPart as any)[e.varName + "Min"] = Math.max(
							(newPart as any)[e.varName + "Min"],
							e.value + 1,
						);
						(currentPart2.part as any)[e.varName + "Max"] =
							Math.min(
								(currentPart2.part as any)[e.varName + "Max"],
								e.value,
							);
