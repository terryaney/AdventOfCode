...
const parseInput = (rawInput: string) => {
	...
	const particles = hailstones.map(hailstone => new Particle3( new Vec3(bignumber(hailstone.px), bignumber(hailstone.py), bignumber(hailstone.pz)), new Vec3(bignumber(hailstone.vx), bignumber(hailstone.vy), bignumber(hailstone.vz) )));
...
class Vec3 {
	constructor(public x0: BigNumber, public x1: BigNumber, public x2: BigNumber) { }
}
class Particle2 {
...
function project(ps: Particle3[], proj: (vec: Vec3) => [BigNumber, BigNumber]): Particle2[] {
	return ps.map(p => new Particle2(
...
function Intersection(p1: Particle2, p2: Particle2): Vec2 | undefined {
	...
	var determinant = p1.vel.x0.times(p2.vel.x1).minus(p1.vel.x1.times(p2.vel.x0));
	...
	return new Vec2(
		divide(p2.vel.x0.times(b0).minus(p1.vel.x0.times(b1)), determinant) as BigNumber,
		divide(p2.vel.x1.times(b0).minus(p1.vel.x1.times(b1)), determinant) as BigNumber
	);
}
function solve2D(particles: Particle2[]): Vec2 {
	...
	const translateV = (p: Particle2, vel: Vec2) =>
		new Particle2(p.pos, new Vec2(p.vel.x0.minus(vel.x0), p.vel.x1.minus(vel.x1)));
	...
	for (let v1 = -s; v1 < s; v1++) {
		for (let v2 = -s; v2 < s; v2++) {
			...
			if (stone != undefined && particles.every(p => Hits(translateV(p, vel), stone))) {
				return stone;
			}
		}
	}
