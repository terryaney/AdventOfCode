...
class Graph {
	...
	constructor(edges: Array<{ from: string, to: string }>, random: () => number) {
		this._verticesCount = Array.from(new Set(edges.flatMap(x => [x.from, x.to]))).length;
		this._edges = edges;
		this._random = random;
	}
	...
	public cut() {
		...
		while (contractedVerticesCount > 2) {
			const edgeToContract = contractedEdges[Math.floor(this._random() * contractedEdges.length)];
			...
			if (contracted[edgeToContract.to]) {
				contracted[edgeToContract.from].push(...contracted[edgeToContract.to]);
		...
		return { minCut: contractedEdges.length, vertices1Count: counts[0], vertices2Count: counts[1] };
	}
}
...
const solve = (rawInput: string, isPart1: boolean) => {
	...
	if (isPart1) {
		...
		let count2 = 0;
		while (minCut !== 3) {
			const { minCut: newMinCut, vertices1Count, vertices2Count } = graph.cut();
			minCut = newMinCut;
