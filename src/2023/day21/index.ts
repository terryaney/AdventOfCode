...
const fillGrid = (grid: string[], startX: number, startY: number, stepsRemaining: number) => {
	// S, . = garden plots
	// # = rock
	// Walk out 'stepsNeeded' to keep spawning markers to where he could reach
	// console.log(lines, start, stepsNeeded);
	
	const tokens: Array<{ x: number, y: number, stepsRemaining: number }> = [{ x: startX, y: startY, stepsRemaining: stepsRemaining }];
	...
	while (tokens.length > 0) {
		...
		[{ x, y: y - 1 }, { x, y: y + 1 }, { x: x - 1, y }, { x: x + 1, y }]
			.filter(({ x, y }) => x >= 0 && y >= 0 && x < grid[0].length && y < grid.length && grid[y][x] != "#" && !seen.has(`${x},${y}`))
			.forEach(location => {
				seen.add(`${location.x},${location.y}`);
				tokens.push({ x: location.x, y: location.y, stepsRemaining: token.stepsRemaining - 1 });
			});
...
const solve = (rawInput: string, testName: string | undefined, isPart2: boolean) => {
	...
	if (!isPart2) {
		...
		*/
		...
		const oddGrids = Math.pow(Math.floor(gridToVisitRightOfStart / 2) * 2 + 1, 2);
		// Grids rounded up to nearest multiple of 2^2
		const evenGrids = Math.pow(Math.floor((gridToVisitRightOfStart + 1) / 2) * 2, 2);
		...
		const oddGridPoints = fillGrid(lines, start.x, start.y, gridWidth * 2 + 1);
		const evenGridPoints = fillGrid(lines, start.x, start.y, gridWidth * 2);

		const vertexStepsRemaining = gridWidth - 1;
		const top = fillGrid(lines, start.x, gridWidth - 1, vertexStepsRemaining);
		const right = fillGrid(lines, 0, start.y, vertexStepsRemaining);
		const bottom = fillGrid(lines, start.x, 0, vertexStepsRemaining);
		const left = fillGrid(lines, gridWidth - 1, start.y, vertexStepsRemaining);
		
		const smallStepsRemaining = Math.floor(gridWidth / 2) - 1;
		const smallTopRight = fillGrid(lines, 0, gridWidth - 1, smallStepsRemaining);
		const smallTopLeft = fillGrid(lines, gridWidth - 1, gridWidth - 1, smallStepsRemaining);
		const smallBottomRight = fillGrid(lines, 0, 0, smallStepsRemaining);
		const smallBottomLeft = fillGrid(lines, gridWidth - 1, 0, smallStepsRemaining);

		const largeStepsRemaining = Math.floor(gridWidth * 3 / 2) - 1;
		const largeTopRight = fillGrid(lines, 0, gridWidth - 1, largeStepsRemaining);
		const largeTopLeft = fillGrid(lines, gridWidth - 1, gridWidth - 1, largeStepsRemaining);
		const largeBottomRight = fillGrid(lines, 0, 0, largeStepsRemaining);
		const largeBottomLeft = fillGrid(lines, gridWidth - 1, 0, largeStepsRemaining);

		return (
			oddGrids * oddGridPoints +
			evenGrids * evenGridPoints + 
			top + right + bottom + left +
			(gridToVisitRightOfStart + 1) * (smallTopRight + smallBottomRight + smallBottomLeft + smallTopLeft) + 
			gridToVisitRightOfStart * (largeTopRight + largeBottomRight + largeBottomLeft + largeTopLeft)
		);
	}
};
