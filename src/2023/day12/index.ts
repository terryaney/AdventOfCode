...
function getArrangements(
	...
	if (conditionPosition == conditionInfo.length) {
		if (sizePosition == sizes.length && damagedSprings == 0) {
			...
			sizePosition == sizes.length - 1 &&
			sizes[sizePosition] == damagedSprings
		) {
		...
		[".", "#"].forEach(condition => {
			...
			if (
				conditionInfo[conditionPosition] == condition ||
				conditionInfo[conditionPosition] == "?"
			) {
...
const parseInput = (rawInput: string, isPart2: boolean) => {
	...
	return lines.map(line => {
		...
		const foldsEnumerable = Array.from(
			{ length: folds },
			(_, index) => index,
		);
		...
		return !isPart2
			...
			: getArrangements(
					{},
					foldsEnumerable.map(f => springs).join("?"),
					0,
					foldsEnumerable
						.map(f => sizes)
						.join(",")
