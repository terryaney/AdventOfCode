...
class Node {
	...
	constructor(
		...
		this.currentDirectionMoves =
			parent == undefined
				? 0
				: this.currentDirection == parent.currentDirection
				? parent.currentDirectionMoves + 1
				: 1;

		this.key = `${this.x},${this.y},${this.currentDirection ?? " "},${
			this.currentDirectionMoves
		}`;
		...
		const isOpposite =
			this.parent == undefined
				...
				: (this.currentDirection == "N" &&
						this.parent.currentDirection == "S") ||
				  (this.currentDirection == "S" &&
						this.parent.currentDirection == "N") ||
				  (this.currentDirection == "W" &&
						this.parent.currentDirection == "E") ||
				  (this.currentDirection == "E" &&
						this.parent.currentDirection == "W");

		const canTurn =
			(parent?.currentDirectionMoves ?? this.currentDirectionMoves) >=
				minMoves ||
			(this.parent?.currentDirection ?? this.currentDirection) ==
				this.currentDirection;
		this.isValid =
			!isOpposite && canTurn && this.currentDirectionMoves <= maxMoves;
	}
}
...
const parseInput = (rawInput: string) => {
	...
	const map = lines.map((line, row) =>
		line.split("").map((char, col) => Number(char)),
	);
...
function findHeatLoss(
	...
	open.push(
		new Node(0, 0, 0, finish.x + finish.y, undefined, minMoves, maxMoves),
	);
	...
	while (open.length > 0) {
		...
		if (
			current.x == finish.x &&
			current.y == finish.y &&
			current.currentDirectionMoves >= minMoves
		) {
			finishNode = current;
			break;
		}
		...
		[
			...
			.forEach(n => {
				...
				if (neighbor.isValid) {
					...
					if (
						(openIndex = open.findIndex(
							n => n.key == neighbor.key,
						)) > -1 &&
						neighbor.cost < open[openIndex].cost
					) {
						open.splice(openIndex, 1);
						openIndex = -1; // we removed the node from the list
					} else if (
						closed.has(neighbor.key) &&
						neighbor.cost < closed.get(neighbor.key)!
					) {
