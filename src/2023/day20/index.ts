...
const parseInput = (rawInput: string) => {
	...
	lines.forEach(line => {
		...
		const module = new Module(
			type == "Conjunction" || type == "FlipFlop"
				? left.substring(1)
				: left,
			type,
			destinations,
	...
	for (const name in modules) {
		...
		module.destinations.forEach(destination => {
			...
			if (
				modules[destination] != undefined &&
				modules[destination].type == "Conjunction"
			) {
	...
	const lcmModules: {
		[name: string]: { cylclesToHigh: number; seen: number };
	} = {};
		...
		while (pulses.length > 0) {
			...
			if (pulse.type == 1) {
				...
				if (pulse.destination == input.rxInput?.name) {
					...
					if (
						isPart2 &&
						Object.values(input.lcmModules).every(m => m.seen > 0)
					) {
						return Object.values(input.lcmModules).reduce(
							(acc, m) => lcm(acc, m.cylclesToHigh / m.seen),
							1,
						);
					}
				}
			...
			if (module != undefined) {
				if (module.type == "FlipFlop" && pulse.type == 0) {
					...
					module.memory.find(i => i.name == pulse.source)!.state =
						pulse.type;
				}
				...
				const pulseToSend =
					module.type == "FlipFlop"
						...
						? module.memory.every(i => i.state == 1)
							? 0
							: 1
						: pulse.type;
				...
				module.destinations.forEach(destination => {
					if (module.type == "FlipFlop" && pulse.type == 0) {
						pulses.push(
							new Pulse(pulseToSend, module.name, destination),
						);
