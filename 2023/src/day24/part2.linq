...
void Main()
{
	...
	sln.PartTwo(File.ReadAllText(@"C:\BTR\OpenSource\AdventOfCode\2023\src\day24\input.txt")).Dump();
}
...
class Solution
{
	...
	Vec2 Solve2D(Particle2[] particles)
	{
		...
		for (var v1 = -s; v1 < s; v1++)
		{
			for (var v2 = -s; v2 < s; v2++)
			{
				...
				if (stone != null && particles.All(p => Hits(translateV(p, vel), stone)))
				{
					return stone;
				}
			}
	...
	Vec2? Intersection(Particle2 p1, Particle2 p2)
	{
		...
		var ba = p1.vel.x0 * p1.pos.x1;
		var bb = p1.vel.x1 * p1.pos.x0;
		var b0 = ba - bb;
